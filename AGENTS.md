...
- `2.0.51+251000` is QQL Build 251, Revision 0: a **shared Course section extraction**. `CoursesScreen` keeps shared Sort, Search and Show unavailable controls; All Courses and Course Studio retain their own data and actions while using common `CourseLibraryCategories`, `CourseLibraryFilter` and `CourseLibrarySection` alongside the existing `CourseLibraryPresentation` and `CourseLibraryRow`. The five categories, learner-scoped Favorites, independent Expanded/Compact state, borders, counts, empty messages, filter and sort behavior, public standalone screens and keys remain unchanged. Course Model v11, package format 1, stored preferences, rights, import, progression, scoring and the Course Editor confirmation remain unchanged. This is a source release without a Windows package. Plan, evidence and handoff: `docs/251_ARCHITECTURE_PLAN.md`, `docs/251_VALIDATION.md` and `docs/251_HANDOFF.md`. Beta expiry is `2026-10-24 23:59:59` local time, recalculated from the 24 September 2026 release date.
- `2.0.50+250001` is QQL Build 250, Revision 1: an owner-requested **Courses layout update**. The Course Manager tab is renamed **Course Studio** in user-facing labels and Help; internal class and key names remain. The Course actions menu stays at the right edge of each row, including narrow layouts, and Course Studio includes **Course Info** in that menu. Tapping a Course's displayed cover or flag opens an enlarged image popup in either tab. Sort by and Show unavailable share one line, with the Search field below Sort when opened. Search is available on both tabs; New course appears only on Course Studio. Each tab has its own Help icon beside its label. Favorites use a soft amber accent without reversing row text and background colors; Course Studio also has a Favorites section for the active learner's Personal Library Courses, using the existing learner-scoped favorite flag. Other Local Courses has a neutral gray section border in both tabs, distinct from red Audit errors, green ready and blue Draft/Unpublished status. Course Model v11, package format 1, stored preferences, rights, import, progression, scoring and the Course Editor's top-level confirmation remain unchanged. Revision 1 is a source release without a Windows package. Plan, evidence and handoff: `docs/250_COURSES_SCREEN_PLAN.md`, `docs/250_VALIDATION.md` and `docs/250_HANDOFF.md`. Beta expiry is `2026-10-24 23:59:59` local time.
- `2.0.50+250000` is QQL Build 250, Revision 0: **Courses** combines All Courses and Course Manager as tabs with shared Course rows, Sort by and Show unavailable controls; All Courses adds Search and Favorites. Favorite and Hide in Learner are per-learner × Course preferences, separate from Personal Library membership. Hide removes a Course from the learner Selector, never the active Course. The Selector groups Current, up to three other Recent, Favorites and Other Courses, and shows locked Course Manager and Course Editor entries greyed out with the profile unlock method. Import returns to its opening surface; Copy and Fork stay unavailable while Course Manager is locked and no longer open the Editor after import. Received Custom Course updates require Personal Library membership, unchanged Maintainer, assigned Team ID and immutable provenance, a strictly newer positive integer Course version, and no local Maintainer or assigned-Team profile; locally authored and signed Publisher update rules remain. `CourseFavoriteService`, `CourseLearnerVisibilityService` and `CourseReceivedService` own the new flags. Course Manager Help is separate from Course Editor Help. Course Model v11, package format 1, signatures, progression, scoring and the Course Editor's top-level confirmation remain unchanged. Plan: `docs/250_COURSES_SCREEN_PLAN.md`. Beta expiry remains `2026-10-23 23:59:59` local time.
...
- Course membership is per opaque learner ID × immutable Course ID. Add/Remove from my courses controls both Selector and Manager inclusion without deleting shared files. The Courses screen has All Courses and Course Manager tabs with the established four Course categories. Hide/Unhide was removed in Build 241 Revision 2, when `course_hidden_` values were ignored and never converted into personal removals. Build 250 Revision 0 reintroduces Hide in Learner with a clean cut on that learner-scoped key: it changes Selector visibility only, never membership, and blocks the active Course; no legacy conversion is attempted because there were no released users as of Build 248. Selector Import returns Home with its result and a Study now action when playable. Lesson display is Expanded / Collapse completed / Focused per learner × Course and initializes Expanded. It operates only on Lesson path visibility, uses authoritative completion/unlock plus IDDQD access, preserves current/last-visited Lesson behavior and never collapses Sections.
...
- In sandboxed Windows sessions where `C:\QQL\flutter\bin\cache` is outside the writable roots, `flutter.bat` can silently spin while retrying creation of `flutter.bat.lock`. If a Flutter command produces no output, inspect the process promptly; stop this specific lock retry and rerun with permission to write the SDK cache. Do not change persistent Windows power or sandbox settings for this workaround.
- During implementation, run the smallest relevant focused tests for rapid feedback.
