...
- `2.0.24+224` rationalizes exercises behind Select, Input, Arrange, Match and Presentation models; adds grouped authoring presets, Type the translation, contextual comprehension, punctuation-safe deterministic answer variants and graded closest-correction feedback; moves the Course Editor Lesson list and unchanged Lock control to a dedicated draft-preserving management page; adds fresh-ID Lesson/Round/Exercise duplication, a registry-backed Exercise Creation Wizard and configurable progressively harder GuideBook Round drafts with explicit approval; and does not change Course Model v5, progression, XP, Review, Duel, identity or existing stable IDs.
