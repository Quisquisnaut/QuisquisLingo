...
- `2.0.34+234000` is QQL Build 234, Revision 0, the final planned Alpha cleanup release. It completes the repository media audit; replaces 92 split Image Bank assets in place and adds editor-only tags; expands the World Flags library with ten language-related flags; adds a shared searchable, language-aware and portable Course flag picker; bundles the Italian-to-Neapolitan `AI-Slop Demo`; replaces startup olive artwork with the canonical QQL logo; fixes the pre-entry Extended flag frame through atomic destination-state loading; preserves and hash-locks all 19 existing audio assets; and adds media integrity validation without changing Course Model v9 or learner persistence. The Alpha expiry is `2026-10-13 23:59:59` local time. A Beta transition belongs only to a later release and remains contingent on QQL 234 validation; see `docs/234_MEDIA_AUDIT.md` and `docs/234_VALIDATION.md`.
