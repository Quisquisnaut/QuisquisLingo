...
- `2.0.28+228` reorganizes Settings and Profile; adds read-only learner Statistics from established activity records; groups Crash and Diagnostic tools under Debug with both files in `Documents/QuisquisLingo/Logs`; and gates learner Round/Duel audio behind per-learner Enable Audio Exercises, initialized Off, before actual source availability and playback initialization. Text-to-speech is also per learner and initialized Off; TTS voice is per learner with System default, and Test Voice speaks only nonblank user-entered text while retaining selected-Course language resolution. This is a clean persistence cut: previous shared and negative audio-setting values remain untouched and unread. A learner listening exercise prepared behind `Before you start` remains silent until Continue makes it active; bounded diagnostics record preparation, UI state, stable exercise ID/type, activation and suppression without content. Actual switches to a different Course show a restrained 680 ms entry fade only when Animations are enabled, reduced motion is absent, and the destination has a valid explicit Course JSON flag; startup, same-Course navigation and missing flags remain immediate without inferred substitutes. Authoring Preview remains setting-independent and no-write. Course Model v6, course JSON, checksums, XP and progression remain unchanged. The 30-day Alpha expiry is `2026-10-08 23:59:59` local time; see `docs/228_VALIDATION.md`.
...
- New Lessons and manually created Rounds may carry the optional v6 `provisionalDraft` marker. Ready marked parents reconcile to non-Draft through canonical authoring changes without redundant parent saves. Explicit Save Draft clears this eligibility; legacy unmarked Drafts, intentional copies/forks and imports converted to Draft remain Draft until explicitly saved. Course delivery stays an independent explicit choice.
