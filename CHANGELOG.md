# 2.0.33 (Phase 233.3, revision 0) - Linux updater, learner Status avatar, and Course/Team governance - 2026-09-14
...
- Phase 233.3 introduces the clean Course Model v8 boundary: every custom Course has an immutable individual Creator and an individual Owner, plus an optional separate assigned Team. Only the current Owner in Edit mode can transfer ownership to another local individual or assign/revoke a Team; a Team can never own or self-assign a Course. Assignment leaves Creator and Owner unchanged and requires a warning describing changing membership/leadership, revocation and the limits of QQL permissions.
- Team governance remains independent and experimental. Team creators begin as Team Leaders; any Team Leader may add/remove members and promote/demote Leaders, while the established final-Leader invariant prevents a Team from reaching zero Leaders. A Team may manage multiple Courses with different Creators and Owners, and Course ownership alone grants no Team-governance power.
- Creator, Owner, Team Leader and Team Member presentation now prefers an optional Discord `@handle`, falls back to Screen Name and continues authorizing solely by opaque internal user ID. The existing Internal IDs control remains the only ID toggle. Course Info separates Creator, Owner, assigned Team, Team Leaders and Team Members without calling the Team an Owner.
...
- Advanced metadata to `2.0.33+233030`, displayed as **Phase 233.3, revision 0**, moved canonical bundled/custom course data and storage to Course Model v8, refreshed all nine official checksums and the validator, and refreshed the established Alpha expiry to `2026-10-14 23:59:59` local time. Earlier custom namespaces remain untouched and unread; learner progression, XP, Review, Duel and course-owned progress formats are unchanged.
