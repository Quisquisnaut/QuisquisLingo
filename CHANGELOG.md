# 2.0.28 (Phase 228, revision 0) - Settings, Statistics, Debug and Audio - 2026-09-08
...
- Restored the restrained Course Entry Animation for actual course switches only. When Animations are enabled, reduced motion is not requested, and the destination has a valid flag explicitly configured in Course JSON, that exact flag covers the Learner Panel for a 680 ms hold-and-fade reveal. Startup, same-course navigation, disabled animations, reduced motion, and missing/invalid JSON flags transition immediately without inferred or generic substitutes.
