...
void main() {
  test(
    'QQL 239 Revision 4 refreshes Beta expiry and includes the expiry day',
    () {
      expect(BetaLifecycleService.expiryIsoDate, '2026-10-19');
      expect(BetaLifecycleService.daysRemaining(DateTime(2026, 9, 19)), 30);
      expect(
        BetaLifecycleService.isExpired(DateTime(2026, 10, 19, 12)),
        isFalse,
      );
      expect(
        BetaLifecycleService.isExpired(DateTime(2026, 10, 19, 23, 59, 59)),
        isFalse,
      );
      expect(BetaLifecycleService.isExpired(DateTime(2026, 10, 20)), isTrue);
    },
  );
  ...
  test('warning milestones are stable', () {
    ...
    expect(BetaLifecycleService.warningStage(DateTime(2026, 10, 18)), 1);
    expect(BetaLifecycleService.warningStage(DateTime(2026, 10, 19)), 0);
  });
