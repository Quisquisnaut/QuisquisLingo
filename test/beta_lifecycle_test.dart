...
void main() {
  test(
    'QQL 241 Revision 1 refreshes Beta expiry and includes the expiry day',
    () {
      expect(BetaLifecycleService.expiryIsoDate, '2026-10-20');
      expect(BetaLifecycleService.daysRemaining(DateTime(2026, 9, 20)), 30);
      expect(
        BetaLifecycleService.isExpired(DateTime(2026, 10, 20, 12)),
        isFalse,
      );
      expect(
        BetaLifecycleService.isExpired(DateTime(2026, 10, 20, 23, 59, 59)),
        isFalse,
      );
      expect(BetaLifecycleService.isExpired(DateTime(2026, 10, 21)), isTrue);
    },
  );
  ...
  test('no-argument lifecycle checks follow the injectable clock', () {
    ...
    BetaLifecycleService.clock = () => DateTime(2026, 10, 19);
    expect(BetaLifecycleService.isExpired(), isFalse);
  ...
  test('warning milestones are stable', () {
    expect(BetaLifecycleService.warningStage(DateTime(2026, 10, 13)), 7);
    expect(BetaLifecycleService.warningStage(DateTime(2026, 10, 17)), 3);
    expect(BetaLifecycleService.warningStage(DateTime(2026, 10, 19)), 1);
    expect(BetaLifecycleService.warningStage(DateTime(2026, 10, 20)), 0);
  });
