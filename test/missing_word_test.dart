...
void main() {
  test('Missing Word round-trips and audits cleanly', () {
    final ex = Exercise(
      id: 'missingword01',
      type: 'missing_word',
      prompt: 'Vorrei comprare un biglietto per Roma.',
      question: '',
      answers: const [],
      correct: null,
      tts: 'Vorrei comprare un biglietto per Roma.',
      accepted: const [],
      tokens: const [],
      orderAnswer: const [],
      pairs: const [],
  ...
  test('Missing Word flags a word absent from transcript', () {
    final ex = Exercise(
      ...
      hint: '',
      icons: const [],
      missingWords: const ['Milano'],
    );
    expect(
      CourseAuditService()
          .auditExercise(ex)
          .any((i) => i.severity == AuditSeverity.error),
      isTrue,
    );
  });
}
