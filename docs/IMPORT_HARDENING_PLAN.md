...
**Status: items 1, 3 and 4 implemented as Revision 14 (`2.0.43+243014`); item 2 (Course package media kept on disk) implemented as Revision 15 (`2.0.43+243015`): media are inflated one entry at a time (never more than one in memory) and staged as files, rather than streamed straight from the inflater to disk.** Choices made: images may sit in any folder of a bank (matched by unique basename); a bank's manifest may also carry a display `name`; the Shared Image Library refuses a bank entry without keywords before writing; Course element limits are 500 Lessons, 100 Rounds per Lesson, 1,000 items per Round or GuideBook and 20,000 recordings; Course packages list at most 20,000 entries.
...
**Status: duplicates, provenance and the date sort implemented as Revision 16 (`2.0.43+243016`), for the Shared Image Library (Course libraries and recordings were already content-addressed). Audio provenance fields were not added: they would change the Course JSON model. Clearer media messages were Revision 17, the preview tooltip Revision 18, and the Phase 20 route matrix Revision 19 (`2.0.43+243019`). That matrix found and closed an embedded Course JSON image-validation gap at import.**
