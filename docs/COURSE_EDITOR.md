...
Updated for Version 2.0.29, Phase 229, revision 1 (technical build 2291) and Course Model v7
...
Course Manager remains the lifecycle hub. Official rows provide **Inspect (read only)**, **Fork as custom course** only when derivative works are explicitly allowed, **Audit**, and **Export JSON** where supported. An individual Owner and every member of an owning Team receive **Edit**, **Duplicate custom course**, **Audit**, **Export JSON**, and manager-only **Delete course**. An outsider receives View/read-only Editor and Course Info, plus Fork only when derivatives are allowed; they never receive direct Edit or Duplicate. Delete retains the established double confirmation and is never offered inside Course Editor. The same policy is enforced again by authoring services, so disabled UI cannot mutate an official or outsider-owned original.
...
## 0.7.3 Course Info Editor roles and languages

Course Info Editor displays Source language and Target language as read-only values. They identify the language used for learner instructions/support and the language being learned. Editing these values is deliberately disabled for now because changing them can affect TTS, instructions, course validation and content semantics.
