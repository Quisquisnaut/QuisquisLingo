...
Updated for Version 2.0.29, Build 229, revision 3 (technical build 2293) and Course Model v7

## Unlocking the editor

Course Manager is an Easter Egg so ordinary learners do not encounter authoring controls accidentally. Open **Settings** and tap/click anywhere in the complete **Version and Build** row ten times. `Course Manager unlocked` appears and the Editor actions become available from the learner **Course Selector**. The unlock is an ordinary setting in the active opaque learner namespace: each learner starts locked, unlocks independently, regains their own state after switching/restart, participates in learner backup/restore, and loses it through normal profile deletion. It changes visibility only and never overrides Course Model v7 Owner/Team permissions. Course Manager is no longer listed in Settings. Course Manager lists and manages courses; Course Editor edits one selected course.

## Capability-driven Editor hierarchy

Bundled official, external official, owned custom, Team-owned, and outsider-owned custom courses use the same Course Editor page and hierarchy. Capability state changes which actions can mutate data; it does not replace the Editor with a conceptually separate inspection UI. Course Info, Audit, Help, IDs, metadata, Version History, Lessons, Rounds, Exercises, and read-only inspection remain available where appropriate.

Course Manager remains the lifecycle hub. Its AppBar icon buttons are the single **Course Import** and **Create new course** controls; their actions, tooltips, accessibility and keyboard behavior are retained without duplicate adjacent text buttons. Official rows provide **Inspect (read only)**, **Fork as custom course** only when derivative works are explicitly allowed, **Audit**, and **Export JSON** where supported. An individual Owner and every member of an owning Team receive **Edit**, **Duplicate custom course**, **Audit**, **Export JSON**, and manager-only **Delete course**. An outsider receives View/read-only Editor and Course Info, plus Fork only when derivatives are allowed; they never receive direct Edit or Duplicate. Delete retains the established double confirmation and is never offered inside Course Editor. The same policy is enforced again by authoring services, so disabled UI cannot mutate an official or outsider-owned original. Each Course row independently shows **Draft** when authored descendants are Draft and **Unpublished** when Course delivery is off. Neither state is inferred from the other; when both apply, the two blue badges appear in Draft/Unpublished order and wrap on narrow layouts.

Fork and Duplicate use the existing authoritative services and remain different operations. Duplicate is only for an authorized Owner/Team member and preserves that ownership. Fork is a licensed outsider derivative with fresh identity and retained source provenance. A created Duplicate or Fork is persisted as its confirmed baseline before its first normal Editor session, so opening and immediately closing it is clean; a real semantic edit still invokes the normal course-level confirmation.
...
Every Course Manager and Course Editor hierarchy AppBar provides the same **Editor Help** action and direct internal-ID toggle. Its current-state tooltip is **Internal IDs hidden. Tap to show** or **Internal IDs shown. Tap to hide**. The device-local preference defaults off, persists globally outside Course JSON, and immediately reveals read-only selectable IDs as secondary text. In Course Info Editor it also reveals the authoritative Owner Team/User ID and the read-only model line obtained from `Course.formatVersion`, for example **Course Model: v7**. IDs remain absent from breadcrumbs; Rename and Move preserve them, while Copy creates fresh IDs.
...
QQL 229 revision 1 uses the clean Course Model v7/build-2291 custom-course namespace. Earlier custom namespaces and Course formats remain physically untouched but are not read or migrated. Every v7 custom course requires an explicit stable Creator profile ID and individual/Team Owner. External official storage loads only its publisher source; bundled loading uses only the bundled asset. Old official overrides remain physically untouched but are not read as content, migrated or converted into forks. A complete custom course is validated structurally before saving and before learner use. Malformed current-namespace JSON is preserved and copied aside before loading fails clearly; it is never silently replaced with an empty collection. Local authoring data has an 8 MB safety limit.
...
Every custom course has separate Creator, Owner, credits, license, and provenance. Creator and Owner use stable internal IDs. Author, Contributor, Illustrator, Team Leader, and custom credit roles are visible descriptive metadata only. They never grant permission. The individual Owner and every member of an owning Team can edit and Duplicate regardless of the selected license. Team Lead status governs Team administration, not ordinary course editing. An outsider cannot edit or Duplicate the original; they may Fork only when derivative works are explicitly allowed. See [Team Manager](TEAM_MANAGER.md).
...
A fork receives fresh course/content IDs, individual ownership by the active profile, and a persisted version-1 baseline before its first ordinary Editor session. It retains the original license text, authorship/credits, parent/version lineage, and existing official/custom provenance. Later changes use ordinary custom backups and version history. A Duplicate likewise receives fresh IDs and a persisted baseline, but is available only to an authorized Owner/Team member; an individual Duplicate remains individually owned and a Team Duplicate remains owned by the same Team. Official updates never merge, rebase, or replace an existing fork.
...
Courses can be imported from and exported to `.json` files without a file chooser. In Course Manager, **Course Import** opens its own page with visible **Import instructions** and the primary **Import Course JSON** button. For import, copy the course file to `Documents/QuisquisLingo/Imports/import.json`, then use that button. The imported course is added under **Local courses** and `import.json` is left in place. Course Audit errors block import; warnings are reported for review but do not block it. Imports must be UTF-8 Course Model v7 JSON and are validated through the normal `Course` parser. Older formats are unsupported and are not read, migrated, converted or deleted; export writes the canonical v7 structure. A custom import must already contain valid explicit Creator and Owner identities; no credit/name fallback is applied. Files larger than 10 MB are refused. Origin collisions are handled explicitly: custom content cannot overwrite an official identity, and external official replacements require the same course ID and declared publisher ID, a newer official version and a valid content checksum. An imported outsider-owned custom course remains read-only; replacement of an existing custom original requires current Owner/Team-member authorization. Publisher authenticity remains unverified in Build 226.01 file imports; installation requires the explicit unverified-publisher warning described above. Exports contain the complete canonical v7 object, including ownership, origin/version metadata and optional custom flag data. Learner **Export my data** remains separate and does not contain courses.
...
**Copy edits as JSON** copies the current working Course Model v7 object to the clipboard and does not create a file. **Export Course JSON** invokes the established exporter to write the complete canonical v7 object to `Documents/QuisquisLingo/Exports`. Export includes stable Creator/Owner identities, credits, license, provenance, versions, and optional custom flag data. Fork exports preserve original publisher, authorship, lineage, license and fork-creator metadata. JSON includes course-owned media metadata and references but does not embed MP3 bytes; verified backups keep their existing referenced-recording copy behavior. Export does not confirm or persist the editing transaction.
