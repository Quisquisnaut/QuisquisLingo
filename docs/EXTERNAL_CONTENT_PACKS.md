...
A Course Model v7 file declares a stable `courseId`, origin, version, source and target languages, text direction, author(s), license, and course data. A custom course additionally requires a stable Creator profile ID and an individual/Team Owner object. Those authorization identities are distinct from visible credits. No older custom format is migrated or assigned an Owner from names. Authors choose the license for each course. Languages are not restricted to a built-in list.
...
An official update must match both the stable course ID and publisher, must be newer, and must have a valid checksum. It archives the previous official source before atomically installing the new official source. Both official origins are locally read-only. Only explicit derivative permission enables an independent custom fork with permanent original authorship/provenance, separate Creator identity and explicit local Owner; missing/forbidden permission blocks the action. The human-readable license remains attached. For a custom import, the individual Owner or owning-Team members can edit and Duplicate regardless of license; all other profiles remain read-only and use the same derivative policy for Fork. Official updates never modify forks. Obsolete Build 225 local overrides are ignored, not migrated or deleted; their backup manifests are not exposed as publisher Version History.
