...
An official update must match both the stable course ID and publisher, must be newer, and must have a valid checksum. It archives the complete active course before atomically installing the exact new official source. Local changes are not merged into the publisher's update and remain recoverable from `Documents/QuisquisLingo/Exports/Course Backups/<courseId>` and Version History.
