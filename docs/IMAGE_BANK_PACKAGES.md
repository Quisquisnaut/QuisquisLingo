...
The manifest accepts the batch fields `id`, `primary_term`, `keywords`, `category`, and `filename`. QuisquisLingo copies the bank into its application-support directory and records local paths for use by exercises and Topics.
...
If an imported bank is later removed while an exercise or Topic still references one of its files, the app displays a missing-image warning rather than silently hiding the problem.
