...
QQL 240 file dialogs (named `Save as…` / `Open from…` since Build 255): macOS uses the same desktop code path as Windows and Linux but is **unverified**. Android uses its own document screens since Build 255 Revision 1 (checked on the Android 16 emulator). Since Revision 2, Android's Quick folders are public in `Download/QuisquisLingo/Imports` and `/Exports`: Quick Export needs no dialog, Quick Import asks once for the Imports folder; checked on Android 16, while Android 10 and Android 7–9 are covered only by mocked tests. **iOS is not supported yet**: the repository has no `ios` project and the dialog buttons stay hidden there. The Quick Export and Quick Import folders always remain available.
