...
Windows is a supported code path. TTS uses System.Speech. QQL itself is `2.0.34+234003`; the Windows package wraps the unchanged `quisquislingo_app.exe` with a separate native x64 bootstrap named `QuisquisLingo.exe`. Packaged users should start the bootstrap, while direct runner and `flutter run` development behavior remain available and unchanged.
...
QQL 240 file dialogs (named `Save as…` / `Open from…` since Build 255): macOS uses the same desktop code path as Windows and Linux but is **unverified**. Android uses its own document screens since Build 255 Revision 1 (checked on the Android 16 emulator). Since Revision 2, Android's Quick folders are public in `Download/QuisquisLingo`, with the same `Import`, `Export`, `Logs` and `ToBeMerged` folders as every desktop since Revision 3: Quick Export needs no dialog, Quick Import asks once for the QuisquisLingo folder; checked on Android 16, while Android 10 and Android 7–9 are covered only by mocked tests. **iOS is not supported yet**: the repository has no `ios` project and the dialog buttons stay hidden there. The Quick Export and Quick Import folders always remain available.
