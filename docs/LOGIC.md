...
All origins use the same capability-driven Course Editor hierarchy. Bundled/external official Courses and outsider custom Courses open read-only; an individual Course Maintainer and every current member of an Assigned Team receive custom-content editing. Only the individual Maintainer can transfer maintenance or assign/revoke a Team. Opening an editable custom Course Editor creates an immutable snapshot of the persisted Course and a separate editable working copy. Every nested authoring operation changes only the working copy. Nested Save/Save as draft never touches the learner-visible Course, creates a backup, or increments a version. Canonical semantic comparison decides whether the complete working copy differs from the snapshot.

Course Model v9 keeps five metadata domains separate. Original Course Creator/Created and fork-specific immediate source/creator/date are immutable provenance. Course Maintainer and optional Assigned Team are operational responsibility. Structured Authors/Contributors are attribution. License and Rights Holder are descriptive legal metadata. Last Version Editor and automatic Modified identify the current version activity. Only Maintainer/Assigned Team membership participates in Course authorization; provenance, attribution and rights records grant no access.

The Maintainer or any Assigned Team member can Edit and Copy as New Course regardless of License; only the Maintainer can transfer maintenance or assign/revoke a Team. Team leadership controls Team membership administration only. An outsider cannot mutate or Copy as New Course from the source and may Fork only when derivative works are allowed. Fork preserves the original lineage and records its immediate source plus Fork Created By/Date, while assigning the active user as Maintainer and omitting Assigned Team. Copy as New Course starts a new lineage with the active user as Original Course Creator/Maintainer/Last Version Editor, a new Original Course Created/Modified timestamp, no fork provenance and no inherited Team. Both operations retain structured attribution, Rights Holder and applicable License without changing legal rights.

Only the top-level Confirm course changes action may persist authoring work. For an existing Course it first creates and verifies a complete versioned backup, then increments the separate internal Course version and atomically writes and verifies the entire working copy. Cancel course changes discards the working copy without backup or version change. Failed backup or persistence preserves the original and keeps the working copy open. Official sources use only their publisher-owned version. Confirming a custom version records Last Version Editor and updates Modified without changing Original Course Creator/Created or fork provenance.
...
- local Course Model v9 custom Courses and external official sources
...
QQL 233.03 moves active custom, external-official, corrupt-backup and bundled-discovery Course persistence to the clean v9/build-233030 namespaces. v8 and older Course data remains physically untouched and unread: no migration, transformation, deletion or fallback read runs. Custom Course JSON requires Original Course Creator/Created lineage and one individual Maintainer, and may include a separate Assigned Team ID. Team membership and leadership state live separately in the verified `quisquislingo_authoring_teams_v1_2291` registry and never enter Course JSON. Course Selector Hide and Lesson expansion mode retain their existing per-learner × Course keys unchanged.
...
Test Voice keeps the two inputs separate: its initially empty dialog supplies only the exact user-entered text, while the selected Course's authoritative learning-language resolution supplies the voice language. Valid general and regional codes are preserved exactly after canonical casing; bundled and custom origins use the same path. An absent, malformed or undefined code uses the established language-name fallback without inventing a region, and `und` is never presented to the learner. Empty and whitespace-only input never reaches the backend; UI locale and text contents do not alter voice resolution. The backend, installed-voice selection, lazy initialization and playback path are unchanged.

The Course Entry Animation is requested only by the two explicit bundled/custom Course switch paths after a destination learner Course is resolved. A valid `worldFlagId`, `flagImageBase64`, or supported `flagCode` explicitly stored on that destination Course remains authoritative in the shared resolution order. When all three fields are absent, Automatic uses the same authoritative learning-language fallback as Course Info, the Course Selector, Learner Page and flag-derived backgrounds. Returning to Automatic removes explicit fields; invalid explicit data and unsupported automatic codes remain neutral and do not invent a flag. With animations enabled and reduced motion absent, the resolved flag covers the newly selected Learner Panel during a restrained two-second fade. Course selection is queued until the Course Selector route has closed, and the overlay controller is created only after selection persistence and the destination learner reload complete. Normal startup, same-Course navigation, disabled animations, reduced motion, and an unsupported fallback install no transition overlay. Course-selection persistence and Flag Background remain independent.

The developer Course Manager unlock is stored beneath the active opaque learner profile prefix. A ten-tap unlock therefore follows only that learner across restarts and ordinary backup/restore; profile switching resolves the new learner independently, new learners default locked, and deletion removes the setting with the rest of that profile namespace. The unlock exposes authoring navigation only and never changes `CourseAccessPolicy` Maintainer or Team capabilities.
