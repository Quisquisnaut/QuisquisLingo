...
Course Model v7 retains the optional presentation and availability fields introduced in Phase 226.04:
...
These defaults preserve their established behavior inside v7. QQL does not migrate or partially load older Course formats. Current copy, transfer, editor transactions and exports retain explicitly stored choices and the complete canonical model.
...
"ownership": {
  ...
  "id": "opaque-uuid-v4-profile-or-team-id"
}
```

An individual Owner ID names one local learner profile. A Team Owner ID names one Team in the separate local Team registry. Owner and Creator IDs must be stable UUIDv4 identities. Author, contributor, illustrator, other credit text, display names, filenames and titles never supply or infer ownership. There is no legacy ownership fallback or migration: a custom file lacking either required field is unsupported.

Team membership and Team Lead status are not Course content and never enter Course JSON. They persist in the verified device-local Team registry and are resolved against `ownership.id` when permissions are evaluated. A Team rename therefore leaves the stable Owner reference unchanged.
...
1. Start with the complete Course Model v7 object serialized by `Course.toJson()` after normal model parsing. Hash the model's serialized values and optional-field rules, not original file bytes or a partial content projection.
...
Course owns an ordered list of Lessons. Every Lesson owns its Guidebook, ordered Rounds and stable Duel identity. Chapter and assessment-Lesson fields are not part of Course Model v7.
...
The former decorative Lesson `imageAsset` field is not part of Course Model v7 and is rejected. It is not an alias for `themeIconAsset` and is not migrated into one. Exercise Content may still use its own image field where that exercise type requires it.

The structural fields `topics`, `topicId` and Lesson `id` are invalid in v7. Opaque stable identifier values from earlier bundled content may retain historical text because changing their values would break references and course-owned progress.
...
Guidebook also accepts optional `insights`, an ordered list of objects containing non-empty string `title` and `text` fields. Absence loads as an empty list and empty lists are omitted from canonical JSON. Insights are edited in the GuideBook working copy and persist only through the existing Save or Save Draft action. Import, export, backup and fork paths carry this optional data with the owning Guidebook; the current `formatVersion` remains 7.
...
Options, tokens and match members are stable Items. Evaluation refers to Item IDs, never display indexes. For `text_match`, v7 writes accepted text as `acceptedAnswers`; legacy `accepted` is rejected rather than adapted.
