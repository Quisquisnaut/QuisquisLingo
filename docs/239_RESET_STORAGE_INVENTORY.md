...
| `<AppSupport>/qql_courses_v1/custom`, `<AppSupport>/qql_courses_v1/external_official` | one JSON file per custom or installed official course; interrupted `.tmp` files also belong to this store | custom courses, everything |
...
`learner_<UUID>_course_library_member_<URI-encoded-course-ID>` is a boolean membership override in SharedPreferences. It never alters Course JSON. The removed `course_hidden_` preferences are ignored, not converted into removals; normal profile/full resets still clear these retired keys. Bundled courses and the creator/maintainer's own Custom Courses default to included; pre-existing course-scoped v4 learner records preserve existing access. Imports explicitly add the importer only. An explicit false overrides these defaults.

Progress reset preserves membership. Profile deletion/non-admin-profile reset removes that profile's membership with its prefix; full reset clears all membership. Course-store reset and individual admin uninstall preserve membership and progress for reinstallation. Inventory includes membership under learner settings. No new file folder is introduced.
