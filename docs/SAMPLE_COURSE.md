...
Updated for Version 2.0.34+234000, Build 234, Revision 0.
...
`tools/regenerate_bundled_courses_225_02.py` is the single deterministic generator. Its `--check` mode regenerates in memory and fails if any tracked asset differs. The two former generator entry points delegate to it so they cannot recreate obsolete formats. `tools/validate_courses.py` validates the exact ten-file registry, v9 schema, timestamps, locale/flag metadata, references, global identity, playability and Duel thresholds.
