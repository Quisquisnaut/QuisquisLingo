...
const Map<String, String> helpEs = {
  ...
  'editorHelp.importCustomCourse.body':
      'Copia un Course ZIP compatible en Documents/QuisquisLingo/Imports/import.zip o, si no tiene medios, un Course Model v11 JSON en import.json. Deja solo uno. El ZIP suele contener course.json, qql-course-package.json y media/ en la raíz; también acepta una carpeta única con el mismo nombre que el ZIP sin .zip, con un aviso. En Course Studio, abre Course Import y elige Import Course package or JSON. QQL comprueba todo el paquete y Course Audit antes de instalar. Los Error bloquean; los Warning se muestran. Las imágenes de Shared Image Library viajan con el curso, sin añadirse a la biblioteca del dispositivo receptor. Un Publisher Course requiere firma aprobada. El archivo original permanece en Imports. JSON debe ser UTF-8 y de hasta 10 MB; el ZIP, de hasta 300 MB comprimido y expandido. No se migran formatos anteriores.',
  'editorHelp.exportCustomCourse.title': 'Exportar un curso personalizado',
  'editorHelp.exportCustomCourse.body':
      'Export Course guarda el JSON completo de Course Model v11 y las imágenes y grabaciones propias referenciadas en un ZIP en Documents/QuisquisLingo/Exports. Los medios incluidos con la app siguen viniendo de QQL. Las imágenes de Shared Image Library usadas por el curso viajan como medios del curso con su ID, etiqueta, categoría, tags, origen y atribución disponible; importar el ZIP no las añade a la biblioteca compartida del destino. El JSON conserva creador, Maintainer, Team, autores, derechos, License, origen, estados Draft/Published y versión. Fork conserva la línea de origen; Copy as New Course crea otra. Save to… puede guardar el mismo ZIP mediante el diálogo del sistema.',
  'editorHelp.importCustomFlag.title': 'Importar una bandera personalizada',
  'editorHelp.importCustomFlag.body':
      'Copia un PNG o JPEG válido en Documents/QuisquisLingo/Exports como flag.png, flag.jpg o flag.jpeg y pulsa Upload custom flag. Si hay varios, QQL usa el primero en ese orden. Límite: 2 MB; dimensiones entre 64 × 40 y 4096 píxeles por lado. QQL verifica el formato y reduce proporcionalmente las imágenes de más de 256 píxeles en su lado mayor. No agranda las pequeñas, no recorta ni añade fondo cuadrado. Conserva la transparencia PNG y convierte el primer fotograma a PNG. Los datos quedan en el Course y sobreviven al exportar, importar o duplicar el JSON. El archivo original permanece. Si falta, está dañado, supera los límites o falla la conversión, la importación se detiene.',
  ...
  'appInfo.exportAndImportLearnerData.body':
      'Profile > User Data > Export my data guarda un backup del perfil activo, con su progreso y preferencias, en Documents/QuisquisLingo/Exports. El nombre es automático; si ya existe, añade _2, _3, etc. Para importar, copia un backup compatible a Documents/QuisquisLingo/Imports/learner_import.json y elige Profile > User Data > Import my data. Los proyectos de Course Editor, Image Bank y Audio Packs son recursos aparte.',
  ...
  'deviceAdminHelp.beforeResetBackups.paragraph1':
      'Profile > User Data exporta solo el perfil activo. Un Admin no puede exportar los datos de otros: pídeles que hagan su backup antes de un Reset que les afecte. Los cursos se exportan uno a uno desde Course Studio. Exports queda en QuisquisLingo/Exports, que Wipe out everything conserva salvo que la desmarques.',
...
Estas builds muestran TEST ONLY. No las distribuyas como versiones públicas; compila la versión normal sin la opción y en una salida limpia. Importa test/fixtures/publishers/dummy-signed-media.zip desde Course Studio > Course Import > Open from… o como Imports/import.zip. Debe confirmarse el editor y la grabación. dummy-signed-v2.json prueba una actualización que elimina ese medio. dummy-unsigned.json y un título firmado alterado deben rechazarse. Una build normal sin la opción rechaza los archivos Dummy como clave desconocida. Estas pruebas no requieren la aprobación de un editor real.''',
  ...
  'exerciseHelp.field.choice.image.body':
      'Añade una imagen al prompt o Context.\n\nQué escribir\nElige una imagen de Shared Image Library o deja un PNG, JPEG o WebP en Documents/QuisquisLingo/Imports/Images y pulsa Import custom image. La importación copia los bytes sin redimensionar ni recortar y no añade la imagen a la biblioteca compartida.\n\nComprobaciones\nMáximo 50 KB; 256 × 256 píxeles y 15 KB son recomendaciones. Solo Image-prompt ordering la exige. Preview comprueba que se vea. Course JSON guarda la ruta, no los bytes.\n\nEjemplo\nassets/exercise_images/house.webp',
  ...
  'exerciseHelp.field.translation_choice_to_target.image.body':
      'Imagen opcional del prompt o Context.\n\nQué escribir\nElige una imagen de Shared Image Library o importa un PNG, JPEG o WebP con Import custom image desde Documents/QuisquisLingo/Imports/Images. Se copia sin cambiar bytes y no entra en la biblioteca compartida.\n\nComprobaciones\nMáximo 50 KB; 256 × 256 píxeles y 15 KB son recomendaciones. Preview debe mostrarla. Course JSON guarda la ruta, no los bytes.',
