...
class CustomCourseTransferService {
  ...
  Future<Course> importCourse() async {
    ...
    if (course.flagImageBase64.length > 1024 * 1024) {
      throw const FormatException('Embedded custom flag data exceed the 1 MB safety limit.');
    }
    if (course.flagImageBase64.isNotEmpty) {
      try {
        base64Decode(course.flagImageBase64);
      } catch (_) {
        throw const FormatException('Embedded custom flag data are not valid Base64.');
      }
    }
    return course;
  }
  ...
  Future<String> exportCourse(Course course) async {
    ...
    if (bytes.length > maxJsonBytes) {
      throw const FormatException('Course JSON exceeds the 10 MB export safety limit.');
    }
    ...
    var output = File('${exportDirectory.path}${Platform.pathSeparator}$baseName.json');
    var suffix = 2;
    while (await output.exists()) {
      output = File('${exportDirectory.path}${Platform.pathSeparator}${baseName}_$suffix.json');
      suffix += 1;
    }
