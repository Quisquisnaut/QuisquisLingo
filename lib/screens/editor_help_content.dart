...
({String title, String body, String note}) editorHelpTechnicalIntro(
  HelpLanguage language,
    ...
    ? (
        ...
        body:
            'Work in progress. Queste pagine descrivono l’implementazione attuale dei Course Model v9/v10, separatamente dalle istruzioni pratiche dell’Editor.',
...
const _italian = <EditorHelpSection>[
  ...
  (
    ...
    body:
        'Export Course JSON salva l’intero JSON di authoring Course Model v9 o v10 direttamente in Documents/QuisquisLingo/Exports. Vengono inclusi Original Course Creator, Original Course Created, Course Maintainer, l’eventuale Assigned Team, Authors / Contributors, Rights Holder, License, la provenienza di fork e merge, lo stato Draft/Published, le date di modifica UTC richieste, i metadati di origine e versione, gli eventuali dati della bandiera personalizzata, i metadati Buy a Coffee, la numerazione delle Lesson e le icone Lesson personalizzate gestite da QQL. Un Fork conserva la discendenza dalla sorgente; Copy as New Course fa partire una discendenza indipendente. I metadati e i riferimenti audio del corso sono inclusi, ma i byte degli MP3 non vengono incorporati; i backup di versione verificati mantengono il loro consueto comportamento di copia delle registrazioni referenziate. Non c’è nessuna finestra Salva con nome.',
  ),
  (
    title: 'Importare una bandiera personalizzata',
    body:
        'Copia un PNG o un JPEG valido in Documents/QuisquisLingo/Exports con il nome flag.png, flag.jpg o flag.jpeg, apri il selettore di bandiere del corso e premi Upload custom flag. Se esistono più nomi tra questi, QQL usa il primo in quest’ordine. Dimensione massima del file: 2 MB (2.097.152 byte). Dimensioni minime dell’immagine: 64 × 40 pixel; massime: 8192 pixel per lato. QQL controlla la firma PNG/JPEG vera e propria e decodifica l’immagine. Le immagini più larghe di 256 pixel sul lato lungo vengono ridotte in proporzione; quelle più piccole ma accettabili non vengono ingrandite. Il primo fotogramma diventa un PNG, senza ritagli e senza forzare un quadrato. La trasparenza già presente nei PNG viene mantenuta; un JPEG non acquista uno sfondo trasparente. I dati PNG vengono incorporati nel corso e sopravvivono all’export/import del JSON e alla duplicazione. Il file di partenza resta dov’è e non serve più. Se il file manca, è illeggibile, non è supportato, è troppo grande, troppo piccolo o troppo grande come risoluzione, l’importazione dà errore; anche una conversione PNG fallita la interrompe.',
  ),
  (
