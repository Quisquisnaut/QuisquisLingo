...
**Current source version: 2.0.27+227030 · Phase 227.03, revision 0 · Course Model v6 (`formatVersion: 6`).**


**QuisquisLingo 2.0.27 Alpha — Phase 227.03 IDDQD state clarity**

Current project version: 2.0.27

Phase 227.03 explains the existing learner IDDQD control without changing it. Off says `Normal progression locks apply.` On says `Locked content can be opened. Normal progression status is preserved.` A genuinely locked Lesson opened through IDDQD keeps its lock marker and shows `Accessible with IDDQD`; its published GuideBook, Rounds and eligible Duel retain the same existing access and availability rules. IDDQD remains Off / On per learner and Course, defaults Off, and toggling it alone changes no progress or XP. The completed Phase 227.02 Small / Off / Extended / Tinted / Inspired Flag Background selector and preserved `soft_inspired` storage identity are unchanged. Theme remains Default / Light / Dark per learner, with Default following live system brightness. See [227.01 baseline](docs/227_01_VALIDATION.md), [227.02 validation](docs/227_02_VALIDATION.md) and [227.03 validation](docs/227_03_VALIDATION.md). Later IDDQD modes, Theme, Settings, Stats and Course Editor work remains deferred.
